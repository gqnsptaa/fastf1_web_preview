...
@app.route("/result", methods=["GET"])   # <-- GET route
def result():
    p = BASE_DIR / "static" / "plot.png"
    plot_url = url_for("static", filename="plot.png", v=int(time.time())) if p.exists() else None
    return render_template("result.html", plot_url=plot_url)

@app.post("/plot")                       # <-- POST route
def plot():
    year = int(request.form.get("year", "2025"))
    event = request.form.get("event", "Baku")
    ...
    plotted = False
    if drv1:
        lap1 = sess.laps.pick_drivers([drv1]).pick_fastest()  # updated API
        tel1 = lap1.get_telemetry()
        ax.plot(tel1["Distance"], tel1["Speed"], label=f"{drv1} fastest")
        plotted = True
    if drv2:
        lap2 = sess.laps.pick_drivers([drv2]).pick_fastest()  # updated API
...
if __name__ == "__main__":
    print(app.url_map)  # sanity-check routes
    app.run(debug=True, use_reloader=False, threaded=False)  # http://127.0.0.1:5000/
